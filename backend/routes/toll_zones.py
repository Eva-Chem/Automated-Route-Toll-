...
# -------------------------------
# CREATE a new toll zone (PROTECTED)
# -------------------------------
...
# -------------------------------
# DISABLE (soft delete) a toll zone (PROTECTED)
# -------------------------------
@toll_zones_bp.route("/toll-zones/<uuid:zone_id>", methods=["DELETE"])
#@jwt_required()
def disable_toll_zone(zone_id):
    """
    Soft delete a toll zone by setting is_active=False.
    Protected endpoint.
    """
    zone = TollZone.query.filter_by(zone_id=zone_id).first()
    if not zone:
